...
- **完整调用链追踪**: 自动分析 A->B->C 这样的完整调用链路（从远程获取代码）
- **代码分析**: 提取变更的类/方法并追踪它们的引用
- **AI 审查**: 将代码上下文和调用链发送给 Claude 进行全面审查
- **项目级别理解**: 不仅仅是 diff - 分析完整影响
...
这样 AI 审查时就会特别关注这些调用链是否会被你的更改影响！
...
1. **获取 Diff**: 从 GitLab/GitHub API 获取分支间的文件变更
   ```
   GET /projects/{id}/repository/compare?from=base&to=branch
   ```

2. **获取仓库所有 Python 文件列表**
   ```
   # GitLab:
   GET /projects/{id}/repository/tree?ref=branch&recursive=1

   # GitHub:
   GET /repos/{owner}/{repo}/git/trees/{ref}
   ```

3. **批量下载文件内容**（用于构建完整调用图）
   - GitLab: `GET /projects/{id}/repository/files/{filepath}?ref={ref}`
   - GitHub: `GET /repos/{owner}/{repo}/contents/{filepath}?ref={ref}`

4. **解析 AST 构建函数调用图**
   - 提取所有函数定义及其调用的函数名
   - 形成 `{(file, func): [called_funcs]}` 的映射关系

5. **向上追溯调用链**
   - 找到被修改函数 C#method3 的所有直接调用者 B#method2
   - 继续向上追溯 A#method1 -> B#method2 -> C#method3
   - 生成完整调用链条

6. **发送给 AI 审查**
   - 将 diff + 调用链 + 相关文件上下文一起发送
   - AI 评估这个调用链是否会被更改影响
